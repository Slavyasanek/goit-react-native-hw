...
export const Post = ({ item }) => {
    ...
    const { photo, title, location, coordinates, likes: { amount, people }, id } = item;
    ...
    return (<View style={styles.post}>
        ...
        <View style={styles.oddInfo}>
            <View style={styles.reactions}>
                <TouchableOpacity style={styles.parametr} onPress={() => navigation.navigate('Comments', { postId: id, photo })}>
                    <Ionicons name="chatbubble" size={24} color={'#FF6C00'} />
                    <Text style={styles.amount}>{commentsCount}</Text>
                </TouchableOpacity>
                <View style={styles.parametr}>
                    <TouchableOpacity onPress={like} disabled={peopleLiked.includes(currentUserId) && true}>
                        {peopleLiked.includes(currentUserId) ? <Ionicons name="thumbs-up-sharp" size={20} color={'#FF6C00'} />
                            : <Feather name="thumbs-up" size={20} color={'#FF6C00'} />}
                    </TouchableOpacity>
                    <Text style={styles.amount}>{postLikes}</Text>
                </View>
            </View>
            <TouchableOpacity style={styles.parametr} onPress={() => navigation.navigate('PostsScreen', {
                    screen: "Map",
                    params: {
